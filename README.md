...
| `character.html` | Night'Flyn's character dashboard: TibiaData profile and highscore row side by side, XP-to-next-level plus milestone context, interactive XP charts (total XP, daily gain, level; 7d/30d/all), full XP/source table, timezone-aware 15-minute online-sample tables, a level-prediction calculator over the tracked pace, level-up logs, skill/highscore standings (magic level, charm points, boss points, achievements, loyalty, fishing, Drome), death table, level-fit hunting decisions and personal hunt-log context |
